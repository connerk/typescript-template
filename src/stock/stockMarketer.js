...
function processTick(ns, stockSymbol) {
  ...
  if (avgTracker.length === 40) {
    ...
    if (profitTracker.volume && profitTracker.position) {
      const stockSaleGain = ns.stock.getSaleGain(
        stockSymbol,
        profitTracker.volume,
        profitTracker.position
      );
      profitPercentage =
        (stockSaleGain - profitTracker.volume * profitTracker.value) /
        (profitTracker.volume * profitTracker.value);
    ...
    if (profitMarginCrossed) {
      const shortSellValue = ns.stock.sellShort(
        stockSymbol,
        9999999999999999999999999n
      );
      if (shortSellValue && profitTracker.volume) {
        const profit =
          profitTracker.volume * (profitTracker.value - shortSellValue) -
          200000;
        corpus += profit;

        const message =
          `[${localeHHMMSS()}] ${stockSymbol}, profitPercentage: ${ns.nFormat(
            profitPercentage,
            '0.0%'
          )}, selling shorts,
          profitTracker.volume: ${
            profitTracker.volume
          }, profitTracker.value: ${ns.nFormat(profitTracker.value, '$0.000a')},
          shortSellValue: ${ns.nFormat(shortSellValue, '$0.000a')},
          profit: ${ns.nFormat(profit, '$0.000a')}, corpus: ${ns.nFormat(
            corpus,
            '$0.000a'
          )}`
            .replace(/\r/g, '')
      ...
      const longSellValue = ns.stock.sell(
        stockSymbol,
        9999999999999999999999999n
      );
      if (longSellValue && profitTracker.volume) {
        const profit =
          profitTracker.volume * (longSellValue - profitTracker.value) - 200000;
        corpus += profit;
      ...
      if (rising) {
        ...
        if (shortSellValue && profitTracker.volume) {
          ...
          const message = `[${localeHHMMSS()}] ${stockSymbol}, selling shorts,
            profitTracker.volume: ${
              profitTracker.volume
            }, profitTracker.value: ${ns.nFormat(
            profitTracker.value,
            '$0.000a'
          )},
            shortSellValue: ${ns.nFormat(shortSellValue, '$0.000a')},
        ...
        const moneyToInvest = firstInvests[stockSymbol]
          ? getMoney(ns)
          : Math.floor(corpus / 6);
        if (moneyToInvest > minimumMoneyToInvest) {
          let volume = Math.floor(
            moneyToInvest / ns.stock.getAskPrice(stockSymbol)
          );
          ...
          if (volume > 0) {
            ...
            ns.tprint(`[${localeHHMMSS()}] ERROR #1: ${stockSymbol}, buying longs, volume: ${volume},
              getMoney(ns): ${getMoney(
                ns
              )}, Math.floor(corpus / 6): ${Math.floor(corpus / 6)},
              moneyToInvest: ${ns.nFormat(moneyToInvest, '$0.000a')}`);
          }
        } else {
          ns.tprint(`[${localeHHMMSS()}] ERROR #2: ${stockSymbol}, buying longs,
            getMoney(ns): ${getMoney(ns)}, Math.floor(corpus / 6): ${Math.floor(
            corpus / 6
          )},
            moneyToInvest: ${ns.nFormat(moneyToInvest, '$0.000a')}`);
        }
        ...
        if (longSellValue && profitTracker.volume) {
          const profit =
            profitTracker.volume * (longSellValue - profitTracker.value) -
            200000;
          corpus += profit;
        ...
        if (moneyToInvest > minimumMoneyToInvest) {
          let volume = Math.floor(
            moneyToInvest / ns.stock.getBidPrice(stockSymbol)
          );
          volume = Math.min(volume, ns.stock.getMaxShares(stockSymbol));
...
export async function main(ns) {
  ...
  stockSymbols.forEach((stockSymbol) => {
    const [sharesLong, avgPriceLong, sharesShort, avgPriceShort] =
      ns.stock.getPosition(stockSymbol);

    corpus += sharesLong * avgPriceLong + sharesShort * avgPriceShort;
  });

  ns.print(
    `[${localeHHMMSS()}] Tick counter: 1, corpus: ${ns.nFormat(
      corpus,
      '$0.000a'
    )}`
  );
  while (true) {
    ...
    if (tickCounter % 10 === 0) {
      ns.print(
        `[${localeHHMMSS()}] Tick counter: ${tickCounter}, corpus: ${ns.nFormat(
          corpus,
          '$0.000a'
        )}`
      );
