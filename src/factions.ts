...
export async function main(ns: NS) {
  if (!ns.singularity)
    return ns.tprint('"factions.js" requires ns.Singularity');
  ...
  /*
	...
	*/
  function remainingFactionAugments(n: string) {
    let factionAugs = factionAugmentation(n);
    factionAugs = filterAugs(factionAugs);
    const augsLength = factionAugs.length;

    for (let i = 0; i < augsLength; i++) {
      while (true) {
  ...
  function localChecklist() {
    const text =
      '\n\n\
		...
		|	----Early Game Factions----	|\n\
		|	CyberSec:		' +
      remainingFactionAugments('CyberSec') +
      '	|\n\
		|	Tian Di Hui:		' +
      remainingFactionAugments('Tian Di Hui') +
      '	|\n\
		|	Netburners:		' +
      remainingFactionAugments('Netburners') +
      '	|\n\
		|					|\n\
		|	-------City Factions-------	|\n\
		|	Sector-12:		' +
      remainingFactionAugments('Sector-12') +
      '	|\n\
		|	Aevum:			' +
      remainingFactionAugments('Aevum') +
      '	|\n\
		|	New Tokyo:		' +
      remainingFactionAugments('New Tokyo') +
      '	|\n\
		|	Chongqing:		' +
      remainingFactionAugments('Chongqing') +
      '	|\n\
		|	Ishima:			' +
      remainingFactionAugments('Ishima') +
      '	|\n\
		|	Volhaven:		' +
      remainingFactionAugments('Volhaven') +
      '	|\n\
		|					|\n\
		|	-------Hacking Groups------	|\n\
		|	NiteSec:		' +
      remainingFactionAugments('NiteSec') +
      '	|\n\
		|	The Black Hand:		' +
      remainingFactionAugments('The Black Hand') +
      '	|\n\
		|	BitRunners:		' +
      remainingFactionAugments('BitRunners') +
      '	|\n\
		|					|\n\
		|	------Megacorporations-----	|\n\
		|	ECorp:			' +
      remainingFactionAugments('ECorp') +
      '	|\n\
		|	MegaCorp:		' +
      remainingFactionAugments('MegaCorp') +
      '	|\n\
		|	KuaiGong International:	' +
      remainingFactionAugments('KuaiGong International') +
      '	|\n\
		|	Four Sigma:		' +
      remainingFactionAugments('Four Sigma') +
      '	|\n\
		|	NWO:			' +
      remainingFactionAugments('NWO') +
      '	|\n\
		|	Blade Industries:	' +
      remainingFactionAugments('Blade Industries') +
      '	|\n\
		|	OmniTek Incorporated:	' +
      remainingFactionAugments('OmniTek Incorporated') +
      '	|\n\
		|	Bachman & Associates:	' +
      remainingFactionAugments('Bachman & Associates') +
      '	|\n\
		|	Clarke Incorporated:	' +
      remainingFactionAugments('Clarke Incorporated') +
      '	|\n\
		|	Fulcrum Technologies:	' +
      remainingFactionAugments('Fulcrum Secret Technologies') +
      '	|\n\
		|					|\n\
		|	---Criminal Organizations--	|\n\
		|	Slum Snakes:		' +
      remainingFactionAugments('Slum Snakes') +
      '	|\n\
		|	Tetrads:		' +
      remainingFactionAugments('Tetrads') +
      '	|\n\
		|	Silhouette:		' +
      remainingFactionAugments('Silhouette') +
      '	|\n\
		|	Speakers for the Dead:	' +
      remainingFactionAugments('Speakers for the Dead') +
      '	|\n\
		|	The Dark Army:		' +
      remainingFactionAugments('The Dark Army') +
      '	|\n\
		|	The Syndicate:		' +
      remainingFactionAugments('The Syndicate') +
      '	|\n\
		|					|\n\
		|	-----Endgame Factions------	|\n\
		|	The Covenant:		' +
      remainingFactionAugments('The Covenant') +
      '	|\n\
		|	Daedalus:		' +
      remainingFactionAugments('Daedalus') +
      '	|\n\
		|	Illuminati:		' +
      remainingFactionAugments('Illuminati') +
      '	|\n\
		|					|\n\
		|	------Special Factions-----	|\n\
		|	Bladeburners:		' +
      remainingFactionAugments('Bladeburners') +
      '	|\n\
		|	Shadows of Anarchy:	' +
      remainingFactionAugments('Shadows of Anarchy') +
      '	|\n\
		└---------------------------------------┘\n\n';
    ns.tprint(text);
  }
  ...
  if (fact == 'CyberSec' || fact == 'cybersec' || fact == 'cyber') {
    ...
    fact == 'Blade-Industries' ||
    fact == 'blade-industries' ||
    fact == 'blade'
  ) {
    factionAugsFunc('Blade Industries');
  } else if (
    fact == 'OmniTek-Incorporated' ||
    fact == 'omnitek-incorporated' ||
    fact == 'omni'
  ) {
    factionAugsFunc('OmniTek Incorporated');
  } else if (
    fact == 'Bachman-Associates' ||
    fact == 'backman-associates' ||
    fact == 'bach'
  ) {
    factionAugsFunc('Bachman & Associates');
  } else if (
    fact == 'Clarke-Incororated' ||
    fact == 'clarke-incorporated' ||
    fact == 'clark'
  ) {
    factionAugsFunc('Clarke Incorporated');
  } else if (
    fact == 'Fulcrum-Secret-Technologies' ||
    fact == 'fulcrum-secret-technologies' ||
    fact == 'fulc'
  ) {
  ...
  function factionAugsFunc(faction: string) {
    const n = faction;
    let factionAugs = factionAugmentation(n);
    factionAugs = filterAugs(factionAugs);
    const augsLength = factionAugs.length;
    ...
    if (factionAugs.length > 0) {
      ...
      output +=
        '\n\n			-----There are no remaining augmentations for faction "' +
        n +
        '"-----\n\n';
    }
