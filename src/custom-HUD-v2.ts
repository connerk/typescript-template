import { NSA } from '/types/index';

export const main = async (ns: NSA): Promise<void> => {
  /*
  ...
  const removeByClassName = (sel: string) =>
    doc.querySelectorAll(sel).forEach((el: HTMLElement) => el.remove());
  const colorByClassName = (sel: string, col: string) =>
    doc
      .querySelectorAll(sel)
      .forEach((el: HTMLElement) => (el.style.color = col));
  ...
  while (true) {
    try {
      ...
      if (gangAPI != false) {
        ...
        gangIncome = ns.formatNumber(
          ns.gang.getGangInformation().moneyGainRate * 5,
          2
      ...
      let cumulative = 0;
      for (const pserv of purchased_servers) {
        let gains = 0;
      ...
      hook0.insertAdjacentHTML(
        'beforeend',
        '<element class="HUD_GN_C HUD_el" title="The name of the City you are currently in.">City </element><br class="HUD_el">'
      );
      colorByClassName('.HUD_GN_C', theme['cha']);
      hook1.insertAdjacentHTML(
        'beforeend',
        `<element class="HUD_GN_C HUD_el">${
          playerCity + '<br class="HUD_el">'
        }</element>`
      );
      colorByClassName('.HUD_GN_C', theme['cha']);
      ...
      hook0.insertAdjacentHTML(
        'beforeend',
        '<element class="HUD_GN_L HUD_el" title="Your current location inside the city.">Location </element><br class="HUD_el">'
      );
      colorByClassName('.HUD_GN_L', theme['cha']);
      hook1.insertAdjacentHTML(
        'beforeend',
        `<element class="HUD_GN_L HUD_el">${
          playerLocation + '<br class="HUD_el">'
        }</element>`
      );
      colorByClassName('.HUD_GN_L', theme['cha']);
      ...
      if (gangInfo != null) {
        ...
        hook0.insertAdjacentHTML(
          'beforeend',
          '<element class="HUD_GN_F HUD_el" title="The name of your gang faction.">Faction </element><br class="HUD_el">'
        );
        colorByClassName('.HUD_GN_F', theme['int']);
        hook1.insertAdjacentHTML(
          'beforeend',
          `<element class="HUD_GN_F HUD_el">${
            gangFaction + '<br class="HUD_el">'
          }</element>`
        );
        colorByClassName('.HUD_GN_F', theme['int']);
        ...
        hook0.insertAdjacentHTML(
          'beforeend',
          '<element class="HUD_GN_R HUD_el" title="The respect of your gang.">Gang Respect</element><br class="HUD_el">'
        );
        colorByClassName('.HUD_GN_R', theme['int']);
        hook1.insertAdjacentHTML(
          'beforeend',
          `<element class="HUD_GN_R HUD_el">${
            gangRespect + '<br class="HUD_el">'
          }</element>`
        );
        colorByClassName('.HUD_GN_R', theme['int']);
        ...
        hook0.insertAdjacentHTML(
          'beforeend',
          '<element class="HUD_GN_I HUD_el" title="The income of your gang.">Gang Income</element><br class="HUD_el">'
        );
        colorByClassName('.HUD_GN_I', theme['int']);
        hook1.insertAdjacentHTML(
          'beforeend',
          `<element class="HUD_GN HUD_el">${
            '$' + gangIncome + '/sec<br class="HUD_el">'
          }</element>`
        );
      ...
      hook0.insertAdjacentHTML(
        'beforeend',
        '<element class="HUD_Karma_H HUD_el" title="Your karma."><br>Karma &nbsp;&nbsp;&nbsp;</element>'
      );
      colorByClassName('.HUD_Karma_H', theme['hp']);
      hook1.insertAdjacentHTML(
        'beforeend',
        `<element class="HUD_Karma HUD_el"><br>${Math.round(
          playerKarma
        )}</element>`
      );
      ...
      hook0.insertAdjacentHTML(
        'beforeend',
        '<element class="HUD_Kills_H HUD_el" title="Your kill count, increases every successful homicide."><br>Kills &nbsp;&nbsp;&nbsp;</element>'
      );
      colorByClassName('.HUD_Kills_H', theme['hp']);
      removeByClassName('.HUD_Kills');
      hook1.insertAdjacentHTML(
        'beforeend',
        `<element class="HUD_Kills HUD_el"><br>${playerKills}</element>`
      );
